...
class Ban(commands.Cog):
    ...
    @commands.slash_command(name='ban', description="Ban a member from your server, with an optional time limit.")
    @commands.has_permissions(ban_members=True)
    async def _ban(self,
                   ctx,
                   member: discord.Option(
                       discord.Member,
                       description="The member of your server to ban."
                   ),
        ...
        logger.info(
            f"{ctx.guild.name} -> {member} was banned!"
        )
        ...
        if duration != 'Permanent':
            ...
            # Unban the member.
            await member.unban(reason=f"Ban duration of {''.join((str(duration[0]), duration[-1]))} has passed!")
            ...
            logger.info(
                f"{ctx.guild.name} -> {member} was un-banned!"
            )
    ...
    async def on_application_command_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            title = "Ban: Error!"
            description = f"Unable to ban: The user has insufficient permissions to ban!"
        ...
        if isinstance(error, commands.BotMissingPermissions):
            title = "Ban: Error!"
            description = f"Unable to ban: The target user has a higher role than the bot!"
