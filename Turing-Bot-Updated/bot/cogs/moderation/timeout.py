...
class Timeout(commands.Cog):
    ...
    async def _timeout(self,
                       ...
                       member: discord.Option(
                           discord.Member,
                           description="The member of your server to time-out."
                       ),
        ...
        # Time the member out.
        await member.timeout_for(datetime.timedelta(seconds=(amount * multiplier[unit])), reason=reason)
        ...
        logger.info(
            f"{ctx.guild.name} -> {member} was timed-out!"
        )
    ...
    async def on_application_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            title = "Time-out: Error!"
            description = f"Unable to time-out: The target user has a higher role than the bot!"
